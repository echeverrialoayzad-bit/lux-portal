...
# -*- coding: utf-8 -*-
"""
Rutas del modulo Cotizaciones FW Fruta
"""
...
from lux_portal.fw_fruta.models import CotizacionFruta as Cotizacion
from lux_portal.fw_fruta.data import AEROLINEAS_LISTA, CARGOS_COMUNES, CARGOS_FREIGHTWISE
...
def dashboard():
    ...
    return render_template('fw_fruta/dashboard.html', cotizaciones=cotizaciones)


@fw_fruta_bp.route('/nueva')
@login_required
def nueva_cotizacion():
    """Formulario para crear nueva cotizacion."""
    return render_template('fw_fruta/form.html', cotizacion=None, aerolineas=[], now=datetime.now())


@fw_fruta_bp.route('/editar/<int:id>')
@login_required
def editar_cotizacion(id):
    """Formulario para editar cotizacion existente."""
    cotizacion = Cotizacion.query.get_or_404(id)
    return render_template('fw_fruta/form.html', cotizacion=cotizacion, aerolineas=cotizacion.aerolineas, now=datetime.now())


@fw_fruta_bp.route('/api/cotizacion', methods=['POST'])
@login_required
...
@fw_fruta_bp.route('/api/cotizacion/<int:id>', methods=['DELETE'])
@login_required
...
@fw_fruta_bp.route('/descargar/<int:id>')
@login_required
def descargar_cotizacion(id):
    ...
    try:
        ...
        if formato == 'pdf':
            ...
            nombre_archivo = f"FWFruta_Cotizacion_{cotizacion.ruta}{sufijo}_{timestamp}.pdf"
            ...
            nombre_archivo = f"FWFruta_Cotizacion_{cotizacion.ruta}{sufijo}_{timestamp}.xlsx"
        ...
        return redirect(url_for('fw_fruta.dashboard'))


@fw_fruta_bp.route('/api/cotizacion/<int:id>')
@login_required
...
@fw_fruta_bp.route('/api/aerolineas')
@login_required
