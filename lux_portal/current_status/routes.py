...
def upload_rates_excel(id):
    ...
    try:
        ...
        AirlineRate.query.filter_by(client_id=client.id).delete(synchronize_session='fetch')
        db.session.flush()
...
def _generar_pdf_cliente(cliente, hide_internal=False, tabla='all'):
    ...
    if tabla in ('all', 'rates'):
        ...
        if len(data1) > 1:
            ...
            # FreightWise Additional Charges sub-table
            ...
            fw_data = [
                [PH('FreightWise Additional Charges:'), PH('')],
                [P('Due Agent'), P(fw_da)],
                [P('Certificate'), P(fw_cert)],
                [P('Phytosanitary'), P(fw_phyto)],
            ]
            total_w = sum(col_widths1)
            fw_table = Table(fw_data, colWidths=[total_w * 0.6, total_w * 0.4])
